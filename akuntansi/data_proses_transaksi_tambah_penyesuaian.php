...
$ref = $_REQUEST["ref"];
$debet = str_replace(".","",$_REQUEST["debit"]);
$kredit = str_replace(".","",$_REQUEST["kredit"]);
...
$kueritrx = "INSERT INTO transaksi (idakun,notrx,nosubtrx,tgl,uraianjurnal,uraianbb,ref,debit,kredit,jurnal) VALUES('$namakun[$i]','$notrx','$subtrx','$tgl','$uraianju[$i]','$uraianbb[$i]','$ref[$i]','$debet[$i]','$kredit[$i]','penyesuaian')";
...
$kueriju = "INSERT INTO jurnalpenyesuaian (kodeakun,nojp,nosubjp,tgl,uraianjurnal,nobukti,ref,debit,kredit,idtrx) VALUES('$namakun[$i]','$notrx','$subtrx','$tgl','$uraianju[$i]','','$ref[$i]','$debet[$i]','$kredit[$i]','$idtrx')";
$prosesju = mysqli_query($connect, $kueriju);


//PROSES BUKU BESAR START
$saldos = "SELECT saldo FROM bukubesar WHERE kodeakun = '$namakun[$i]' AND tgl = '$tgl' ORDER BY tgl DESC LIMIT 1";
$resultdos = mysqli_query($connect, $saldos);
$rowdos = mysqli_fetch_row($resultdos);
if($rowdos[0] == ""){
	//ambil nilai saldo start
	$cektgldb = "SELECT idbb,tgl,saldo FROM bukubesar WHERE kodeakun = '$namakun[$i]' AND tgl < '$tgl' GROUP by tgl ORDER BY idbb DESC";
	$resulttgldb = mysqli_query($connect, $cektgldb);
	$count = mysqli_num_rows($resulttgldb);
	if($count > 1){
		$saldovalue = "SELECT saldo FROM bukubesar WHERE kodeakun = '$namakun[$i]' AND tgl < '$tgl' ORDER BY tgl DESC LIMIT 1";
	}else{
		$saldovalue = "SELECT saldo FROM bukubesar WHERE kodeakun = '$namakun[$i]' AND tgl < '$tgl' ORDER BY idbb DESC LIMIT 1";
	}
...
echo "<script type='text/javascript'>window.location.href='data_transaksi_penyesuaian.php';</script>";
