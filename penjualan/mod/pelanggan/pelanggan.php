...
include"lib/conn.php";
	...
	//link buat paging
	$linkaksi = 'med.php?mod=pelanggan';
	...
	$aksi = 'mod/pelanggan/act_pelanggan.php';
	...
	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				...
				$query = mysqli_query($conn,"SELECT * FROM kios WHERE kode = '$_GET[id]'");
				...
				{
					header("location:med.php?mod=pelanggan");
				}

			}
			else
			{
				$act = "$aksi?mod=pelanggan&act=simpan";
			}
			...
			echo"<form class='w3-small' method='POST' action='$act'>
				<table>
					<tr>
						<td width='220px'><label class='w3-label'>KODE KIOS</label></td>
						<td width='10px'>:</td>
						<td><input type='text' name='kode' class='w3-input' placeholder='kode_kios' value='"?><?php echo isset($c['kode']) ? $c['kode'] : '';?><?php echo"'"?> <?php echo isset($c['kode']) ? ' readonly' : ' ';?><?php echo" required>
						</td>
						
					</tr>
					<tr>
						<td><label class='w3-label'>KIOS</label></td>
						<td>:</td>
						<td><input type='text' name='nama' class='w3-input' placeholder='nama_kios' value='"?><?php echo isset($c['nama']) ? $c['nama'] : '';?><?php echo"' required>
						</td>
					</tr>
				...
				<tr>
					<td>
						<form class='w3-tiny' action='' method='GET'>	
							<input type='hidden' name='mod' value='pelanggan'>
							<div class='w3-row'>
								<div class='w3-col s1'>
					...
					<td align='right'><a href='med.php?mod=pelanggan' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='med.php?mod=pelanggan&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysqli_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td>$m[alamat]</td>
							<td><a href='med.php?mod=pelanggan&act=form&id=$m[kode]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a> 
							<a href='$aksi?mod=pelanggan&act=hapus&id=$m[id]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a></td>
