	...
	switch ($act) {
		case 'form':
			...
			if(!empty($_GET['id']))
			{
				...
				$query = mysqli_query($conn,"SELECT * FROM tb_user WHERE id_user = '$_GET[id]'");
		...
		default :
			...
			<table class=\"w3-table w3-striped w3-bordered w3-tiny w3-hoverable tbl\">
				...
				$sql_kel = mysqli_query($conn,$query);
				...
				if($fd_kel > 0)
				{
					...
					while ($m = mysqli_fetch_assoc($sql_kel)) {
						echo"<tr>
							...
							<td>$m[last_login]</td>
							<td>
							<a href='med.php?mod=user&act=form&id=$m[id_user]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a>
							<a href='$aksi?mod=user&act=hapus&id=$m[id_user]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a></td>
