<?php
	...
	$aksi = 'mod/barang/act_barang.php';

	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				$act = "$aksi?mod=barang&act=edit";
				$query = mysqli_query($conn,"SELECT * FROM tb_barang WHERE kode_barang = '$_GET[id]'");
				$temukan = mysqli_num_rows($query);
				if($temukan > 0)
				{
					$c = mysqli_fetch_assoc($query);
					$stok_masuk = stok_masuk($c['kode_barang']);
					$stok_keluar = stok_keluar($c['kode_barang']);
					$total_stok = ($c['jml_stok'] + $stok_masuk) - $stok_keluar;
				}
				else
				{
					header("location:med.php?mod=barang");
				}

			}
			else
			{
				$act = "$aksi?mod=barang&act=simpan";
			}
			...
			echo"<form class='w3-small' method='POST' action='$act'>
				...
				<table>
					<tr>
						...
						<td width='10px'>:</td>
						<td><input type='text' name='id' class='w3-input' placeholder='kode_barang' value='"?><?php echo isset($c['kode_barang']) ? $c['kode_barang'] : '';?><?php echo"'"?> <?php echo isset($c['kode_barang']) ? ' readonly' : ' ';?><?php echo">
						</td>
					...
					<tr>
						...
						<td><div class='w3-row'>
							<div class='w3-col s4'>
								...
								$sql_kategori = mysqli_query($conn,"SELECT * FROM tb_kategori_barang");
					...
					<tr>
						...
						<td><input type='text' name='jml_stok' class='w3-input' placeholder='stok' value='"?><?php echo isset($total_stok) ? $total_stok : '';?><?php echo"' required>
						</td>
					</tr>
					<tr>
						<td><label class='w3-label'>SATUAN</label></td>
						<td>:</td>
						<td><input type='text' name='satuan' class='w3-input' placeholder='satuan' value='"?><?php echo isset($c['satuan']) ? $c['satuan'] : '';?><?php echo"' required>
						</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
		...
		default :
			...
			echo"<table style='margin-top:12px;'>
				<tr>
					...
					<td align='right'><a href='med.php?mod=barang' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='med.php?mod=barang&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
