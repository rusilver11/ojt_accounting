<?php
	...
	$aksi = 'mod/supplier/act_supplier.php';

	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				$act = "$aksi?mod=supplier&act=edit";
				$query = mysqli_query($conn,"SELECT * FROM tb_supplier WHERE kode_supplier = '$_GET[id]'");
				...
				{
					header("location:med.php?mod=supplier");
				}

			}
			else
			{
				$act = "$aksi?mod=supplier&act=simpan";
			}
			...
			echo"<form class='w3-small' method='POST' action='$act'>
				<table>
					...
					<tr>
						<td><label class='w3-label'>NAMA SUPPLIER</label></td>
						<td>:</td>
				...
				<tr>
					<td>
						<form class='w3-tiny' action='' method='GET'>	
							<input type='hidden' name='mod' value='supplier'>
							<div class='w3-row'>
								<div class='w3-col s1'>
								...
								<div class='w3-col s2'>
									<select name='field' class='w3-select w3-padding'>
										...
										<option value='nama_toko'>NAMA SUPPLIER</option>
					...
					<td align='right'><a href='med.php?mod=supplier' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='med.php?mod=supplier&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysqli_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td>$m[telepon]</td>
							<td>$m[email]</td>
							<td><a href='med.php?mod=supplier&act=form&id=$m[kode_supplier]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a> 
							<a href='$aksi?mod=supplier&act=hapus&id=$m[kode_supplier]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a></td>
