<?php
	...
	$aksi = 'mod/kategori/act_kategori.php';
	...
	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				...
				$query = mysqli_query($conn,"SELECT * FROM tb_kategori_barang WHERE kategori_id = '$_GET[id]'");
				...
				{
					header("location:med.php?mod=kategori");
				}
		...
		default :
			...
			echo"<table style='margin-top:12px;'>
				<tr>
					...
					<td align='right'><a href='med.php?mod=kategori' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='med.php?mod=kategori&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysqli_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td><a href='med.php?mod=kategori&act=form&id=$m[kategori_id]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a> 
							<a href='$aksi?mod=kategori&act=hapus&id=$m[kategori_id]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a></td>
