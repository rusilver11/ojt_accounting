	...
	switch ($act) {
		...
		case "form" :
			echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
				...
				<p style='margin-top:0;padding-top:0;'><i>Input Data Pembelian Pupuk</i></p>
			</div>";
			...
			echo"<div class='w3-row-padding'>
				<div class='w3-col s3 w3-card'>
					...
					<div class='w3-card-2 w3-light-blue'>
						<form action='$aksi?mod=pembelian&act=add' method='POST' class='w3-container'>
							...
							<br><input type='text' name='barang' id='barang' placeholder='ketik nama pupuk ...' class='w3-input w3-tiny w3-border-0' required>

							<label class='w3-label w3-text-black'>Harga Beli (Rp):</label>
							<input type='text' name='harga' id='harga' placeholder='ketik nama pupuk ...' class='w3-input w3-tiny w3-border-0' required>
							<p>
							<div class='w3-row'>
				...
				<div class='w3-col s9'>
					...
					if(mysqli_num_rows($sql) > 0)
					{
						...
						{
							$sub_total = $p['harga_beli'] * (($p['qty']*1000)/50);
					...
					<div class='w3-row-padding'>
						...
						<div class='w3-col s5'>
							<form action='$aksi?mod=pembelian&act=simpan' method='POST'>
								<input type='text' name='no_faktur' id='no_faktur' class='w3-input w3-tiny' placeholder='Nomor Faktur / Nomor DO ...' required>
		...
		case "detail" :
			...
			{
				echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
					<h4 style='margin-bottom:0;padding-bottom:0;'>Detail Pembelian Pupuk</h4>
					<p style='margin-top:0;padding-top:0;'><i>Data Detail Pembelian Pupuk Baru</i></p>
				</div><br>
				...
				$sqltrans = mysqli_query($conn,"SELECT * FROM tb_pembelian WHERE no_faktur = '$_GET[id]'") or die(mysqli_error());
				$tra = mysqli_fetch_assoc($sqltrans);

				echo"<table class='w3-table w3-tiny'>
					<tr style='border-bottom:1px dashed #ccc;'>
						<td width='150px'>No. DO</td>
						<td width='10px'>:</td>
				...
				$sql = mysqli_query($conn,"SELECT a.*, b.nama_barang, b.satuan 
									...
									ON a.kode_barang = b.kode_barang
									WHERE a.no_faktur = '$_GET[id]'") or die(mysqli_error());
				$total = 0;
				$no = 1;
				while($p = mysqli_fetch_assoc($sql))
				{
...
<script type="text/javascript">
	$(function(){
		...
		<?php
			$sqlTags = mysqli_query($conn,"SELECT b.*,k.nama_kategori FROM tb_barang b JOIN tb_kategori_barang k
ON b.kategori_id = k.kategori_id ORDER BY nama_barang ASC") or die(mysqli_error());

			$tags = array();
			while($t = mysqli_fetch_assoc($sqlTags))
			{
				$tags[] = '{label : "'.$t['nama_barang'] ." -  ". $t['nama_kategori'] .'", value : "'.$t['kode_barang'].'"}';
			}
		?>
	    ...
	    <?php
			$sqlTags = mysqli_query($conn,"SELECT b.*,k.nama_kategori FROM tb_barang b JOIN tb_kategori_barang k
ON b.kategori_id = k.kategori_id ORDER BY nama_barang ASC");

			$tagsHgr = array();
			while($t = mysqli_fetch_assoc($sqlTags))
			{
				$tagsHgr[] = '{label : "'.$t['nama_barang']." - Rp. ". $t['harga_beli'] .'", value : "'.$t['harga_beli'].'"}';
			}
		?>
		var availableTags = [<?php echo implode(", \n\t\t\t", $tagsHgr); ?>];
	    $( "#harga" ).autocomplete({
	    ...
	    <?php
			$sqlTags = mysqli_query($conn,"SELECT * FROM tb_supplier 
								ORDER BY nama_toko ASC");

			$tagss = array();
			while($s = mysqli_fetch_assoc($sqlTags))
			{
